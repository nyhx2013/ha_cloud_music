...
class VlcDevice(MediaPlayerDevice):
    ...
    def interval(self, now):
        ...
        if self._media != None:
            _log('当前时间：%s，当前进度：%s,总进度：%s,源进度：%s', self._media_position_updated_at, self._media_position, self.media_duration, self._media.attributes['media_position'])
            ...
            if self._timer_enable == True:
                ...
                if self._source_list != None and len(self._source_list) > 0 and self.media_duration > 0 and ((self.media_duration > 3 and self.media_duration - 3 <= self.media_position) or (self._state != STATE_PLAYING and self.media_duration == 0 and self._media_position  == -3)) and self.next_count > 0:
                    _log('播放器更新 下一曲')
